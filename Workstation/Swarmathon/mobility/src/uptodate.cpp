		...
		switch(stateMachineState) {
			...
			case STATE_MACHINE_TRANSFORM: {
				...
				//Otherwise, assign a new goal
					...
					if(currentLocation.x<-5){ //el if #3
						goalLocation.x = -5-currentLocation.x;
						goalLocation.y = currentLocation.y;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
						} //le fi #3
					else if(currentLocation.y<-5){ //el if #4
					goalLocation.y = -5-currentLocation.y;
						goalLocation.x = currentLocation.x;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
					} //le fi #4
					

					else if(currentLocation.y>5){ //el if #6
					goalLocation.y = 5-currentLocation.y;
						goalLocation.x = currentLocation.x;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
					} //le fi #6
					else if(currentLocation.x>5){ //el if #4
					goalLocation.x = 5-currentLocation.x;
						goalLocation.y = currentLocation.y;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
					} //le fi #4
					...
					int gridWgtArray [12]= {1};	
					int kk=0;
					
					for(int ii;ii<6;++ii){
						for (int jj;jj<1;++jj){
					//gridWgtArray [kk] = gridWgt [(xmax+1)/2+tempx+ii-1][(ymax+1)/2+tempy+jj-1];
					 gridWgtArray [kk] = gridWgt [ii+1][1+jj+2*(myID-1)];				
					kk +=1;
					}
					}


					sort(gridWgtArray, gridWgtArray + 12);
				if(gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy] !=gridWgtArray[0]){
						for(int ii;ii<6;++ii){
						for (int jj;jj<1;++jj){
					

					if ( gridWgt [ii+1][1+jj+2*(myID-1)] ==gridWgtArray[0]){
					goalLocation.x = gridID_x [ii+1];
					goalLocation.y = gridID_y [1+jj+2*(myID-1)];
					goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);						
					break;
					}
 					}
   ...
   //Todo     
			...
			int tempx;
			 ...
			 tempy = round(target_detail.y/2);
                         gridWgt [(xmax+1)/2+tempx] [(ymax+1)/2+tempy] *=2;
			 for (int ii;ii<2;++ii){
				for (int jj;jj<2;++jj){
