		...
		switch(stateMachineState) {
			...
			case STATE_MACHINE_TRANSFORM: {
				...
				//Otherwise, assign a new goal
					...
					if (! ((currentLocation.x<0) && (fabs(currentLocation.x)>currentLocation.y)) )	//if#1		
					{// if current location is not insided the desired area
					 if(currentLocation.y>=0){ 
						goalLocation.x = -currentLocation.y;
						goalLocation.y = -currentLocation.x;					
						}else{
					...
					else if(currentLocation.x<-4){ //el if #3
						goalLocation.x = -4-currentLocation.x;
						goalLocation.y = currentLocation.y;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
						} //le fi #3
					else if(currentLocation.y<-4){ //el if #4
					goalLocation.y = -4-currentLocation.y;
						goalLocation.x = currentLocation.x;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
					} //le fi #4
					else{ //el#1
					...
					int tempx;
			 		...
			 		tempx = round(goalLocation.x/2);
			
			 		tempy = round(goalLocation.y/2);
					int gridWgtArray [5]= {gridWgt [(xmax+1)/2+tempx+1][(ymax+1)/2+tempy],gridWgt [(xmax+1)/2+tempx-1][(ymax+1)/2+tempy],gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy+1],gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy-1],gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy]};
					sort(gridWgtArray, gridWgtArray + 5);
					if (gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy] != gridWgtArray [0])
						{
						if (gridWgt [(xmax+1)/2+tempx+1][(ymax+1)/2+tempy]== gridWgtArray [0])
						  {					
					

					//select new position 50 cm from current location
  		
					goalLocation.x = gridID_x [(xmax+1)/2+tempx+1];
					goalLocation.y = gridID_y[(ymax+1)/2+tempy];
					goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	}
						else if(gridWgt [(xmax+1)/2+tempx-1][(ymax+1)/2+tempy]== gridWgtArray [0])
						 {goalLocation.x = gridID_x [(xmax+1)/2+tempx-1];
					goalLocation.y = gridID_y[(ymax+1)/2+tempy];
					goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	}
						else if(gridWgt [(xmax+1)/2+tempx][(ymax+1)/2+tempy+1]== gridWgtArray [0])
						{goalLocation.x = gridID_x [(xmax+1)/2+tempx];
					goalLocation.y = gridID_y[(ymax+1)/2+tempy+1];
					goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	}
						else
						{goalLocation.x = gridID_x [(xmax+1)/2+tempx];
					goalLocation.y = gridID_y[(ymax+1)/2+tempy-1];
					...
					if (! ((currentLocation.y>0) && (fabs(currentLocation.x)<fabs(currentLocation.y))) )	//if#1		
					{// if current location is not insided the desired area
					...
					else if(currentLocation.y> 4){ //el if #3
						goalLocation.y = 4-currentLocation.y;
						goalLocation.x = currentLocation.x;
						goalLocation.theta = atan2(goalLocation.y - currentLocation.y, goalLocation.x - currentLocation.x);	
						} //le fi #3
					...
					if (! ((currentLocation.x>0) && (currentLocation.x>fabs(currentLocation.y))) )	//if#1		
					{// if current location is not insided the desired area
					 if(currentLocation.y>=0){ 
						goalLocation.x = currentLocation.y;
						goalLocation.y = currentLocation.x;					
...
void targetHandler(const shared_messages::TagsImage::ConstPtr& message) {
   ...
   //Todo     
			...
			int tempx;
			 ...
			 tempx = round(target_detail.x/2);
			
			 tempy = round(target_detail.y/2);
                         gridWgt [(xmax+1)/2+tempx] [(ymax+1)/2+tempy] = gridWgt [(xmax+1)/2+tempx] [(ymax+1)/2+tempy]*2;
			 for (int ii;ii<2;++ii){
				for (int jj;jj<2;++jj){
			 gridWgt [(xmax+1)/2+tempx+ii-1] [(ymax+1)/2+tempy+jj-1] *= 1.2;
			}
			}
